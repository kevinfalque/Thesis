...
\subsection{Exciton-exciton non linear interactions}
...
On the other hand, bright excitons result from interband transitions since both photon absorption and emission involve an electron changing band. \textcolor{red}{RAJOUTER QQ CHOSE ICI}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.8\linewidth]{chap1/fig/InGaAs.pdf}
...
From the previous sections it appears obvious that two excitons can interact through the Coulomb interaction between their constitutive charged particles.
However, even in the absence of any coulomb potential two excitons can interact through the so called fermionic exchange. 
...
\begin{figure}[]
    \centering
    \includegraphics[width=0.8\linewidth]{chap1/fig/polariton_cavity.pdf}
...
\begin{figure}[]
    \centering
    \includegraphics[width=1\linewidth]{chap1/fig/wedge_and_hopfields.pdf}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.75\linewidth]{chap1/fig/anticrossing.pdf}
...
\textbf{One wavefunction to describe them all.} A pioneering result in the early stages of quantum mechanics is the so called wave-particle duality proposed by Louis de Broglie \cite{deBroglie1925} in 1924.
...
\textbf{Driven dissipative Gross-Pitaevskii Equation.}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\linewidth]{chap1/fig/polariton_bec.pdf}
...
To avoid first complications due to cross Kerr effect we consider a ring cavity of geometrical length $L_{cav}^0$ filled with Kerr medium of lenght $l$ as shown in \autoref{fig:optical_bistability} a). 
As explained for a planar cavity in \autoref{sec:photon}, the cavity has transmission peaks whenever the optical path of a round trip in the cavity (a photon going back and forth) $L_{cav}$ is an integer multiple of the laser wavelength $\lambda$.
...
\end{equation}
where $F$ is the cavity finesse and $k=2\pi/\lambda$ the wavevector of the laser. The transmission of the cavity as a function of the laser frequency is plotted in \autoref{fig:optical_bistability} b).
