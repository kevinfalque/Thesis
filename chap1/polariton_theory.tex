...
\begin{equation}
    \Gamma_{i\to f}^{(2)} = \dfrac{2\pi}{\hbar} \bigg \vert \sum_{m} \dfrac{\bra{f}V_q\ket{m}\bra{m}V_q\ket{i}}{E_i - E_n}\bigg \vert^2 \delta(E_f - E_i)
...
\bigskip
\noindent \textbf{Dressing intraband scattering with interband jumps }



To clarify this idea let us consider this time the scattering between a conduction and a valence band electron. 
The same scattering process can also occur in two steps. First, a conduction band electron is scattered in its band while a valence band electron is promoted to the conduction band. Since an electron changes band it brings a potential $\propto 1/q$. Secondly, 
 the excited electron goes back to the valence band while a valence band electron is scattered in its band yielding again a potential $\propto 1/q$. This two steps interaction end up with the same initial and final states than direct intraband scattering but involves both the creation and the annihilation of an exciton.  In other words the overall process conserves the number of electrons in each band but is made of two not number-conserving transitions.
 As it combine two scatterings behaving as $1/q$ potentials the overall potential behaves also as $1/q^2$ but with an additionnal $1/E_{gap}$ factor. More specifically the two steps contribution is : $\left( 4\pi e^2/L^3q^2 \right)\left[ (e^2/L^3\kappa^2)/E_{gap}\right]$.  
It is possible to arbitrary increase the number of intermediate states by adding "bubbles" of exchange interaction in between the two steps : a previously excited conduction band electron returns to the valence band while a valence band electron is promoted to the conduction band.
...
\noindent  This decomposition through intermediate states can be easily understood using the Feynman diagram of \autoref{fig:intra_b} b). From this, we can already see
 how interbands jumps can dress the direct intraband scattering and reduce
 
 This intermediate "bubbles" are referred as electron-hole exchanges and can occur multiple time.  
in \autoref{fig:inter_band_bubbles}.
